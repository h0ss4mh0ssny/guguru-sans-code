...
def get_options():
    ...
    for arg in sys.argv[1:]:
        ...
        elif arg == "--regular-only":
            options["regular-only"] = True
...
def delete_duplicate_glyphs(jp_font, eng_font):
    ...
    # LATIN 系グリフには Google Sans Code を使用
    for glyph in jp_font.glyphs():
...
def transform_half_width(jp_font, eng_font):
    ...
    # Google Sans Code 縮小の影響でアンダースコアを並べると密接してしまうので、少し離す
    underscore = eng_font[0x005F]
    underscore.transform(psMat.scale(0.95, 1))
    underscore.transform(psMat.translate((after_width_eng - underscore.width) / 2, 0))
    underscore.width = after_width_eng
...
def make_box_drawing_full_width(eng_font, jp_font):
    ...
    jp_font.selection.none()
    jp_font.mergeFonts(
        fontforge.open(f"{SOURCE_FONTS_DIR}/AdjustedGlyphs/FullWidthBoxDrawings.sfd")
    )
    # 幅設定と位置調整
